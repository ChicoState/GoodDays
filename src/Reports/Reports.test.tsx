...
describe("Reports Component", () => { //groups tests of reports
  ...
  it("will it render the Reports component correctly?", () => {
    ...
    expect(wrapper.find("h2").text()).toBe("Reports Section"); // testing reports section
    expect(wrapper.find(LineChartComponent)).toHaveLength(7); //checks number of LineCharts components that are created
  });
});